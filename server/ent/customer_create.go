...
func (cc *CustomerCreate) SetEmailAddress(s string) *CustomerCreate {
	cc.mutation.SetEmailAddress(s)
	return cc
}
...
func (cc *CustomerCreate) check() error {
	...
	if _, ok := cc.mutation.EmailAddress(); !ok {
		return &ValidationError{Name: "email_address", err: errors.New(`ent: missing required field "Customer.email_address"`)}
	}
	return nil
}
...
func (cc *CustomerCreate) createSpec() (*Customer, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.EmailAddress(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
