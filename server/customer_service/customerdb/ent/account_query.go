...
func (aq *AccountQuery) QueryOwner() *CustomerQuery {
...
func (aq *AccountQuery) WithOwner(opts ...func(*CustomerQuery)) *AccountQuery {
	query := &CustomerQuery{config: aq.config}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withOwner; query != nil {
		...
		for i := range nodes {
			if nodes[i].customer_account == nil {
				continue
			}
			fk := *nodes[i].customer_account
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "customer_account" returned %v`, n.ID)
			}
			for i := range nodes {
