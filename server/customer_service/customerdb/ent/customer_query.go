...
func (cq *CustomerQuery) QueryAccount() *AccountQuery {
...
func (cq *CustomerQuery) WithAccount(opts ...func(*AccountQuery)) *CustomerQuery {
	query := &AccountQuery{config: cq.config}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Customer, error) {
	...
	if query := cq.withAccount; query != nil {
		...
		query.Where(predicate.Account(func(s *sql.Selector) {
			s.Where(sql.InValues(customer.AccountColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.customer_account
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "customer_account" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "customer_account" returned %v for node %v`, *fk, n.ID)
			}
